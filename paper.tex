...
To make the estimation tractable, we parameterize $G_{t}(\cdot |X;\theta )$ and $F_{t}(\cdot |X;\theta )$ into location-scale families. This parameterization allows us to use nonlinear least squares estimation, which is less computationally costly than maximum likelihood estimation.
...
In practice, builder participation may vary across slots, which we acknowledge as a limitation in Section~\ref{sec:conclusion}. In our implementation, we assume that the number of type-$t$ bidders is known fixed at $n_{t}$ across all auctions in the sample. Our non-linear least squares estimation proceeds in two steps:\textit{\medskip }
...
\noindent \textit{Step 2}. For $t=1,2$, estimate the common shock volatility parameters $\theta_t^\sigma$ by
plugging $\omega _{t}(X;\hat{\theta})$ from the first step into the sample
analog of the variance expression in (\ref{eq:variance_orderstat}). Specifically, we estimate $\theta_t^\sigma$ via least squares based on equation (\ref{eq:variance_orderstat}).\footnote{We may also pool the moments from \eqref{eq:location_scale_iden} and \eqref{eq:variance_orderstat}     together, and run a single step of Generalized Method of Moment (GMM) estimation. Nonetheless, this two-step algorithm is computationally easier, and valid, because the solution of $\theta_t^\mu,\theta_t^\omega$ in \eqref{eq:location_scale_iden} is unique and the solution of $\theta_t^\sigma$ in \eqref{eq:variance_orderstat} is unique (after plugging in $\theta_t^\omega$ recovered from Step 1).}
...
The estimates reveal economically sensible patterns. For integrated builders, the coefficient on price volatility ($\hat{\mu}_1 = 0.0646$) is larger than for non-integrated builders ($\hat{\mu}_1 = 0.0402$), which aligns with our expectation that integrated builders derive more value from CEX/DEX arbitrage opportunities that arise from price movements. Both types show positive sensitivity to gas prices ($\hat{\mu}_2 = 0.0143$ for integrated and $\hat{\mu}_2 = 0.0160$ for non-integrated), suggesting that network congestion affects all builders through similar channels. The exponential functional forms capture non-linear relationships between observables and builder values. We define the reported ``overall'' R$^2$ as the average of the Step 1 and Step 2 R$^2$ values, where each step uses the standard $1-\text{SSR}/\text{SST}$ definition in its corresponding least-squares moment equation. By this metric, the model achieves an overall R$^2$ of 0.1094 for integrated builders and 0.0485 for non-integrated builders. Although these values are modest, they are consistent with the high idiosyncratic noise and substantial slot-level heterogeneity in MEV-Boost auctions, and the model still captures the key comparative patterns across builder types.

\begin{table*}[t]
\centering
\caption{Estimation Results by Builder Type}
...
\cline{3-4}
\textbf{Parameter} & \textbf{Regressor} & \multicolumn{1}{c}{\textbf{Estimate (95\% CI)}} & \multicolumn{1}{c}{\textbf{Estimate (95\% CI)}} \\
\hline

\multicolumn{4}{l}{\textit{Panel A: Level} \quad $\mu(X)=\mu_1 X_1+\mu_2 X_2+\mu_3$} \\
\hline
$\mu_1$ & Abs. Price Change (bps)  & 0.0646 \quad (0.062,\;0.067) & 0.0402 \quad (0.035,\;0.040) \\
$\mu_2$ & Gas Price (Gwei)         & 0.0143 \quad (0.014,\;0.015) & 0.0160 \quad (0.014,\;0.016) \\
$\mu_3$ & Constant                 & -3.4444 \quad (-3.460,\;-3.428) & -3.8956 \quad (-3.865,\;-3.833) \\
\hline

\multicolumn{4}{l}{\textit{Panel B: Idiosyncratic scale} \quad $\omega(X)=\exp\{\omega_1 X_1+\omega_2 X_2+\omega_3\}$} \\
\hline
$\omega_1$ & Abs. Price Change (bps) & -0.0120 \quad (-0.024,\;0.001) & 0.0357 \quad (-0.034,\;0.008) \\
$\omega_2$ & Gas Price (Gwei)        & -0.3913 \quad (-0.426,\;-0.347) & -0.9673 \quad (-0.648,\;-0.409) \\
$\omega_3$ & Constant                & 0.4734 \quad (0.325,\;0.585) & 0.7140 \quad (0.068,\;0.814) \\
\hline

\multicolumn{4}{l}{\textit{Panel C: Common-shock volatility} \quad $\sigma(X)=\exp\{\sigma_1 X_1+\sigma_2 X_2+\sigma_3\}$} \\
\hline
$\sigma_1$ & Abs. Price Change (bps) & 0.0311 \quad (0.028,\;0.034) & 0.0197 \quad (0.014,\;0.018) \\
$\sigma_2$ & Gas Price (Gwei)        & 0.0048 \quad (0.002,\;0.006) & -0.0236 \quad (-0.004,\;0.003) \\
$\sigma_3$ & Constant                & -0.9652 \quad (-1.009,\;-0.915) & -0.1481 \quad (-0.553,\;-0.453) \\
\hline\hline
\end{tabular}

\vspace{6pt}
\parbox{\linewidth}{\footnotesize\emph{Notes:} Entries report point estimates with 95\% bootstrap confidence intervals (1,000 replications).
``Abs.\ Price Change (bps)'' is the absolute ETH--USD price change on Binance over the prior 12-second slot, measured in basis points (1 bps = 0.01\%).
``Gas Price (Gwei)'' is the base fee per gas of the block, where 1 Gwei = $10^{-9}$ ETH.}
...
To illustrate how our estimates translate into block-specific value distributions, consider a randomly selected block in our sample (block 20,915,921, slot 10,125,585) with $X_1 = 2.21$ basis points (absolute ETH-USD price change) and $X_2 = 19.40$ Gwei (base fee per gas).\footnote{Our code makes it easy to compute these distributions for any block in the sample.} Using our parameter estimates, we estimate the distribution parameters:

\begin{align*}
\hat{\mu}_1(X) &= 0.0646 \times 2.21 + 0.0143 \times 19.40 - 3.4444 = -3.024,\\
\hat{\omega}_1(X) &= \exp\{-0.0120 \times 2.21 - 0.3913 \times 19.40 + 0.4734\} = 0.0005,\\
\hat{\sigma}_1(X) &= \exp\{0.0311 \times 2.21 + 0.0048 \times 19.40 - 0.9652\} = 0.448,
\end{align*}

for integrated builders, and

\begin{align*}
\hat{\mu}_2(X) &= 0.0402 \times 2.21 + 0.0160 \times 19.40 - 3.8956 = -3.497,\\
\hat{\omega}_2(X) &= \exp\{0.0357 \times 2.21 - 0.9673 \times 19.40 + 0.7140\} = 0.00001,\\
\hat{\sigma}_2(X) &= \exp\{0.0197 \times 2.21 - 0.0236 \times 19.40 - 0.1481\} = 0.570,
\end{align*}

for non-integrated builders. Under the log-normal distribution assumption, these parameters imply exponential of expected log values of $\exp(\hat{\mu}_1(X) + 0.5\hat{\sigma}_1^2(X)) = 0.054$ ETH for integrated builders and $\exp(\hat{\mu}_2(X) + 0.5\hat{\sigma}_2^2(X)) = 0.036$ ETH for non-integrated builders. The higher mean for integrated builders reflects their advantage in capturing arbitrage opportunities, while the larger $\hat{\sigma}_2(X)$ indicates greater common-shock variance for non-integrated builders.
...
In this subsection, our primary focus is on revenue implications for proposers under a first-price sealed-bid format rather than on allocative efficiency.
