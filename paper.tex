...
The remainder of this paper proceeds as follows. Section 1.1 explains the sources of value in block building (MEV opportunities) and the heterogeneity between integrated and non-integrated builders. Section 1.2 describes the MEV-Boost auction mechanism. Section 2 presents our structural model for estimating builder value distributions. Section 3 describes our data sources and construction. Section 4 presents our preliminary estimation results and their interpretation. Section 5 concludes with a discussion of data limitations, future improvements, and planned counterfactual analyses.
...
construction, 
\[
P_{t}^{(r:n_{t})}=C_{t}+\epsilon _{t}^{(r:n_{t})}\text{ for }r\leq n_{t}-1%
\text{ and }t=1,2\text{.} 
\]
We exclude the highest order statistic because the winning bid is censored: $P_{j}\leq V_{j}$, so the top order statistic is not observed.

Conditional on $X$, $C_{t}$ is independent from $\epsilon _{t}^{(r-1:n_{t})}$
and $\epsilon _{t}^{(r:n_{t})}$, where $\epsilon _{t}^{(r:n_{t})}$ is the $r$%
-th smallest order statistic out of $n_{t}$ \textit{independent} draws from
the \textit{same} type-$t$ distribution of $\epsilon _{i}$ given $X$, i.e., $%
F_{t}(\cdot |X)$. 

The identification argument proceeds as follows. We observe multiple order statistics from the same type (e.g., the second-highest, third-highest, etc. bids from type-$t$ builders). Each order statistic is the sum of the common component $C_t$ (shared by all type-$t$ builders) and an idiosyncratic order statistic $\epsilon_t^{(r:n_t)}$. Because the common component affects all bidders of the same type identically, while the idiosyncratic components are independent across bidders, we can use the variation across order statistics to separate these two components. Specifically, using the deconvolution method from \cite{cho2024deconvolution}, we can recover the distribution $G_{t}(\cdot |X)$ and $F_{t}(\cdot |X)$ up to a location normalization. The key insight is that by observing multiple order statistics from the same type, we can separate the common component (which affects all bidders of that type) from the idiosyncratic component (which varies across individual bidders). The location normalization arises because we can only identify the distributions up to an additive constant, which is standard in this literature. In our estimation, we normalize by setting the mean of the common component to zero when all observables are at their sample means.\textit{\medskip }

To make the estimation tractable, we parameterize $G_{t}(\cdot |X;\theta )$ and $F_{t}(\cdot |X;\theta )$ into location-scale families. This parameterization is standard in the empirical auctions literature and allows us to use nonlinear least squares estimation, which may be less computationally costly than maximum likelihood estimation.

Under the location-scale parameterization, we can write $C_{t}$ and $\epsilon _{i}$ (with $i$
being type-$t$) as:%
\begin{eqnarray*}
C_{t} &\equiv &\mu _{t}(X;\theta _{t}^{\mu })+\sigma _{t}(X;\theta
_{t}^{\sigma })\times \upsilon _{t}\text{, }\sigma _{t}(\cdot )>0\text{;} \\
\epsilon _{i} &\equiv &\omega _{t}(X;\theta _{t}^{\omega })\times \eta _{i}%
\text{, }\omega _{t}(\cdot )>0\text{,}
\end{eqnarray*}%
where $\upsilon _{1},\upsilon _{2},\eta _{1},...,\eta _{n},X$ are \textit{%
mutually independent} with $\upsilon _{t},\eta _{i}$ drawn from known,
standardized parametric distributions with \textit{zero mean} and \textit{%
unit variance}. 
It then follows that for any $r_{t}\leq n_{t}-1$:%
\begin{equation}
E\left( \left. P_{t}^{(r_{t}:n_{t})}\right\vert X\right) =E(C_{t}|X)+E\left(
\left. \epsilon _{t}^{(r_{t}:n_{t})}\right\vert X\right) =\mu _{t}(X;\theta
...
Likewise, we can write the conditional variance of $P_t^{(r:n_{t})}$ as:
\begin{equation}
  V\left( \left. P_{t}^{(r_{t}:n_{t})}\right\vert X\right) =\sigma
  _{t}^{2}(X;\theta _{t}^{\sigma })+\omega _{t}^{2}(X;\theta _{t}^{\omega
  })\times \delta _{(r_{t}:n_{t})}\text{,}
  \label{eq:variance_orderstat}    
\end{equation}

For simplicity, we assume that the number of type-$t$ bidders is fixed at $n_{t}$ across all auctions in the sample. In practice, we handle variation in participation by conditioning on the observed number of bidders. We use all available order statistics (not just the second-highest) to improve efficiency. That is, we use $r_{t}=1,2,\ldots,n_{t}-1$ for each type $t$. 

The non-linear least squares estimation proceeds in two steps:\textit{\medskip }
...
\sum_{r=1}^{n_{b,(t)}-1}\left[ p_{b,t}^{(r)}-\mu _{t}(x_{b};\theta
...
analog of the variance expression in (\ref{eq:variance_orderstat}). Specifically, we use the sample variance of order statistics and solve for $\sigma_t$ using equation (\ref{eq:variance_orderstat}).\textit{\medskip }

We use standard bootstrap to calculate confidence intervals. The bootstrap procedure resamples auctions with replacement and re-estimates the parameters on each bootstrap sample. We use 1,000 bootstrap replications and report percentile-based confidence intervals. This approach accounts for sampling variation and potential correlation across auctions, and is standard in the empirical auctions literature \citep{hendricks2007empirical}.

To summarize, our model decomposes builder values into common and idiosyncratic components, where the common component captures type-specific factors (e.g., CEX price movements affecting all integrated builders similarly) and the idiosyncratic component captures builder-specific private information. Under our assumptions, we can separately identify the distributions of both components using order statistics from the bid data. The estimation proceeds in two steps: first, we estimate the mean and idiosyncratic scale parameters using the conditional mean of order statistics; second, we estimate the common shock volatility using the conditional variance. This approach allows us to recover the full distribution of builder values, which is necessary for counterfactual analysis.

\section{Data}
To estimate the model parameters described in Section 2, we require data on bids, builder identities, and observable characteristics that affect block value. We describe our data sources and construction below.

The data used in this paper is sourced from 95,000 MEV-Boost auctions sampled from selected days between October and December 2024. This sample size provides sufficient variation for reliable estimation of the model parameters. The sampling strategy selects representative days across the three-month period to capture variation in market conditions while maintaining computational tractability. In each of these slots, bids from the active relays are aggregated, with the winning bid selected by matching the attached block hash of bids to the hash of the block that made it on-chain. 

We identify the winning bidder and other participants in the auction by mapping their bidding addresses to publicly known builder identities. This mapping is based on public sources including builder announcements, relay operator disclosures, and blockchain analytics. Based on observed behavior patterns (e.g., responsiveness to CEX price movements) and known connections with high-frequency trading firms, we categorize builders as either integrated or non-integrated. In our sample, we observe 3 integrated builders (Rsync, Beaverbuild, and Titan) and 2 non-integrated builders consistently participating across auctions. These five builders account for over 75\% of blocks in our sample period. 
...
Within each auction, we track several variables alongside the winning bid and the bids of other prolific bidders. We track the base fee per gas of the block, which reflects network congestion at the time of the bid and the potential profits collected by the builder from transaction fees. Gas is the unit of computational work on Ethereum, and gas prices (measured in Gwei, where 1 Gwei = $10^{-9}$ ETH) represent the fees users pay to have their transactions processed. The gas fee in our sample ranges from 6.38 Gwei at the 25th percentile to 19.92 Gwei at the 75th percentile, with a median of 12.15 Gwei, highlighting substantial volatility in network congestion. The number of transactions in the mempool at the time of the block is also available, including the number of transactions which would have paid the base fee. This figure represents the demand for blockspace during each slot, as each transaction over the base fee is a potentially profitable inclusion for a builder with available space in their block. While the median percent of transactions in the mempool that pay the base fee is only 17 percent, there are periods of high demand for blockspace where this figure is significantly higher, near 100 percent. 

Finally, we track the absolute price movements of the ETH-USDC pair on Binance over the 12 seconds of each slot. High volatility periods provide more arbitrage opportunities, leading to significantly higher bids in the MEV-Boost auction from integrated builders. For example, the maximum winning bid amount is four orders of magnitude higher than the 75th percentile winning bid amount. These extreme bids occur when there are large arbitrage opportunities available, and are evidence of the ``lottery blocks'' that are extremely valuable to proposers. 
...
\section{Results}
Using the data described above, we estimate the model parameters. We present our results below.
...
We use three explanatory variables in our current specification: the absolute price change of ETH-USD on Binance in the preceding 12 seconds ($X_1$), the gas price of the block measured in Gwei ($X_2$), and a constant term. Additional observables (e.g., mempool composition, time-of-day effects) could be incorporated to improve model fit, but we focus on these core variables to establish the baseline estimates. 
 
We assume the following functional forms for the location and scale parameters:
\begin{align*}
...
\multicolumn{4}{l}{\textit{Panel A: Level} \quad $\mu(X)=\exp\{\mu_1 X_1+\mu_2 X_2+\mu_3\}$} \\
\hline
...
Starting with the level parameters (Panel A), the positive and statistically significant coefficients on $X_1$ (absolute ETH price change) for both builder types confirm that price volatility is a key driver of block value. The larger coefficient for integrated builders (0.0646 vs. 0.0402) reflects their comparative advantage in capturing CEX/DEX arbitrage opportunities, as discussed in Section 1. This differential response is economically significant: a 1-unit increase in absolute price change multiplies the expected common component by $\exp(0.0646) \approx 1.067$ (a 6.7\% increase) for integrated builders versus $\exp(0.0402) \approx 1.041$ (a 4.1\% increase) for non-integrated builders. The positive coefficients on gas price ($\mu_2 = 0.0143$ for integrated and $\mu_2 = 0.0160$ for non-integrated) indicate that network congestion increases builder values, likely through its impact on transaction fees and blockspace demand, with similar sensitivity across types.

The idiosyncratic scale parameters (Panel B) show that $\omega_2$ is negative and significant for both types ($\omega_2 = -0.3913$ for integrated and $\omega_2 = -0.9673$ for non-integrated), suggesting that higher gas prices reduce the variance of builder-specific private information. This may reflect that during high congestion periods, the set of profitable transactions becomes more standardized, reducing heterogeneity in builder valuations. The constant term $\omega_3$ is positive for both types (0.4734 for integrated and 0.7140 for non-integrated), indicating substantial idiosyncratic variation in builder values even after conditioning on observables.

The common-shock volatility parameters (Panel C) reveal interesting patterns. The positive coefficients on $X_1$ ($\sigma_1 = 0.0311$ for integrated and $\sigma_1 = 0.0197$ for non-integrated) suggest that higher price volatility increases the variance of the common shock component, which is consistent with price movements creating more variable arbitrage opportunities. The coefficient on gas price ($\sigma_2$) is positive for integrated builders (0.0048) but negative for non-integrated builders (-0.0236), suggesting differential effects of congestion on common shock variance across types. The negative constant terms ($\sigma_3 = -0.9652$ for integrated and $\sigma_3 = -0.1481$ for non-integrated) indicate that the common shock has relatively low variance on average, which is consistent with builders of the same type sharing similar information about block value. The fact that $\sigma_3$ is more negative for integrated builders suggests that integrated builders share more similar information about block value, possibly because they all observe the same CEX price movements and face similar arbitrage opportunities.

The negative constant terms in the level parameters ($\mu_3$) reflect the baseline (log) expected value when both $X_1$ and $X_2$ are zero. The fact that these are negative and more negative for non-integrated builders (-3.8956) than for integrated builders (-3.4444) suggests that integrated builders have higher baseline values, with differences emerging both through baseline levels and differential responses to observables like price volatility.
...
Our main findings are threefold. First, we document substantial heterogeneity between builder types: integrated builders (affiliated with high-frequency trading firms) derive significantly more value from price volatility than non-integrated builders, with a 1-unit increase in absolute price change multiplying expected values by approximately 6.7\% for integrated builders versus 4.1\% for non-integrated builders. This differential response aligns with integrated builders' comparative advantage in capturing CEX/DEX arbitrage opportunities. Second, both builder types respond positively to network congestion (gas prices), with similar sensitivity ($\mu_2 = 0.0143$ for integrated and $\mu_2 = 0.0160$ for non-integrated), suggesting that congestion increases builder values through similar channels (transaction fees and blockspace demand). Third, our structural estimates reveal interesting patterns in the variance components: higher gas prices reduce idiosyncratic variation (suggesting more standardized transaction sets during congestion), while price volatility increases common shock variance (suggesting more variable arbitrage opportunities during volatile periods).
...
\item \texttt{Titan\_highest\_bid}, \texttt{Flashbots\_highest\_bid}: Highest bids from neutral builders
...
\item \texttt{2nd\_highest\_bid\_type\_2\_log}: Log of second-highest bid from neutral builders
\end{itemize}

The data construction process involves aggregating bids across multiple relays, identifying the winning bid by matching block hashes, and computing order statistics (second-highest bids) for each builder type within each auction.
...
The dataset is too large for GitHub and is available from the authors upon request. The code is well-documented with function docstrings explaining each step of the estimation procedure, following the methodology described in Section 2 of this paper.
